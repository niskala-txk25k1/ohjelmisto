...
function open_modal(meta)
{
	this.querySelector("img").src = meta.image.large;
	this.querySelector("img").alt = meta.title;
	this.showModal();
}

for (const meta of picArray) {

	const article = document.createElement("article");
	article.addEventListener("click", open_modal.bind(dialog, meta));
	article.classList.add("card");
	{
		const h2 = document.createElement("h2");
		h2.innerHTML = meta.title;
		article.appendChild(h2);

		const figure = document.createElement("figure");
		{
			const img = document.createElement("img");
			img.src = meta.image.medium;
			img.alt = meta.title;
			figure.appendChild(img);

			const figcaption = document.createElement("figcaption");
			figcaption.innerHTML = meta.caption;
